...
def streaming(token, message, mediatype, choice):
	...
	if conn.auth:
		if choice == 1: 
			...
			deepseek_response = ''.join(core.deepseek(message, False))
			random = randint(0, 1)
			...
			return StreamingResponse(core.deepseek(message, True), media_type=mediatype)
		else:
			if choice == 2:
				return StreamingResponse(core.openai(message, True), media_type=mediatype)
			else:
...
def non_streaming(token, message, mediatype, choice):
	...
	if conn.auth:
		if choice == 1:
			...
			return core.deepseek(message, False)
		else:
			if choice == 2:
				return core.openai(message, False)
			else:
